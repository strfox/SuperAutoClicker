...
QString getStringNameFor(keycomb_t keycomb) {
    ...
    case (VK_XBUTTON2):
        ...
        break;
    default:
        keyname = QString("UNKNOWN (0x%1)").arg(QString::number(keycomb.vkCode, 16));
        break;
    }
...
keycomb_t parseKeyComb(QString str) {
    ...
    if (tokens.size() != 5) {
        static const char* formatstr = "Expected 5 tokens while parsing key combination but got %d. Input: %s";
        char buf[1024];
        snprintf(
                 buf,                 //       char   *__stream
                 sizeof(buf),         //       size_t  __n
